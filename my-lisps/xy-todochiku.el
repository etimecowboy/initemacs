;;; xy-todochiku.el - My hack based on 
;;;    todochiku.el - A mode for interfacing with Growl, Snarl, and
;;;    the like.
;; (defconst todochiku-version "0.0.8")
(defconst xy-todochiku-version "0.0.8")
...
(defcustom todochiku-command 
  (case system-type 
    ...
    (windows-nt "C:/Program Files/growl for windows/growlnotify.exe")
...
This would be better done through a customization probably."
  (case system-type
	...
	('windows-nt (list (format "/t:\"%s\"" title) (format "/i:\"%s\"" (todochiku-icon icon)) message ;; (if sticky "/s:sticky")
					   ))
