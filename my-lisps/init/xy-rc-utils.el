...
;; (qiang-set-font
;;   '("Consolas" "Courier" "Monaco" "DejaVu Sans Mono" "Monospace" "Courier New") ":pixelsize=14"
;;   '("Microsoft Yahei" "文泉驿等宽正黑" "文泉驿等宽微米黑" "黑体" "新宋体" "宋体") 16))

;; 设置字体 Emacs 会优先选用 Concolas + “雅黑”的组合。
;; 如果“雅黑”没有装的话，就使用“文泉驿等宽正黑”，依此类推。
;; 这份字体配置不用改动就能在不同的操作系统字体环境下面使用。
;;
;; 另注：中文要用大一点的字体，使中文字符的宽度正好等于两倍（整数倍）英文字符，
;; 才能配合 org mode 下的 table，不至于对不齐。
;; 测试：
	...
	(message "* ---[ `%s' exists and is newer. ]---" generated-autoload-file))
      ...
      (when with-subdirs
        (while files
          ...
          (when (file-directory-p dir-or-file)
            (if recursive
                (xy/update-all-autoloads dir-or-file 
