...
(defun gnus-settings ()
  ...
  ;;窗口的显示？可以通过调用函数 gnus-add-configuration 来控制窗口的配
  ;;置。语法有点复杂，不过在手册 "Windows Layout" 中解释得很清楚，一些
  ;;比较流行的例子：
  ;;1. 用 35% 的 Summary 比 65% 的 Article 替换原来的 25% 比 75%（其中
  ;; 的1.0 意思是“占满剩余空间”）：
  ;;
  ;;(gnus-add-configuration '(article (vertical 1.0 (summary .35 point)
  ;; (article 1.0))))
  ;;
  ;;2. 三个窗口显示。左边是 Group buffer，右上是 Summary buffer，右下是
  ;;Article buffer：
  ...
  (gnus-add-configuration
   '(article (vertical 1.0 (summary 0.35 point) (article 1.0))))
  ...
  ;; 注意这个函数必须放在 gnus-summary-line-format 设置之前，否则 gnus
  ;; 无法理解后者而出错
  ...
  ;;                  "\\|"))
  ;; TODO: 将原本的hook改成直接设置setq效果如何？有问题吗？
...
archive
  ...
  ;;另一种保存帖子的方法：
  ;;当然，也可以使用缓冲：(setq gnus-use-cache t)这样，你只需设置 tick
  ;;或者 dormant 标记来保存，在缓冲中设置已读标记可以删除（文章）看到有
  ;;价值的帖子，只要按下`*'键，这篇帖子就会被拷贝到本地的cache中保存起
  ;;来，即使服务器那边删除了帖子，也没关系了。如果不想要了，用`Meta-*'
  ;;就可以把帖子从缓存中删掉。
